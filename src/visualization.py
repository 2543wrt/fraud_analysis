...
def create_visualizations(df_results, feature_importance, graph):
    ...
    plt.tight_layout()
    save_path_risk = os.path.join(output_dir, 'risk_analysis.png')
    plt.savefig(save_path_risk, dpi=150, bbox_inches='tight')
    print(f"  ✓ Saved {save_path_risk}")
    ...
    plt.axis('off')
    plt.tight_layout()
    save_path_network = os.path.join(output_dir, 'high_risk_network.png')
    plt.savefig(save_path_network, dpi=150, bbox_inches='tight')
    print(f"  ✓ Saved {save_path_network}")